...
new Handle:g_CvarInfectedVision;
new Handle:g_CvarSpectatorVision;
...
public OnPluginStart()
{
	...
	OUR_COLOR[2] = 255;
	// Setup Clone Array
	g_ArrayHittableClones = CreateArray();
	g_ArrayHittables = CreateArray();
	
	g_CvarInfectedVision = CreateConVar("tanksupport_infected_all", "0", "Whether to enable the entire Infected Team to have vision of tank hittables", _, true, 0.0, true, 1.0);
	g_CvarSpectatorVision = CreateConVar("tanksupport_spectator", "0", "Whether to enable the Spectator Team to have vision of tank hittables", _, true, 0.0, true, 1.0);

	HookConVarChange(g_CvarInfectedVision, OnConVarVisionChanged);
	HookConVarChange(g_CvarSpectatorVision, OnConVarVisionChanged);
...
public OnConVarVisionChanged(Handle:convar, const String:oldValue[], const String:newValue[])
{
	for (int i = 1; i <= MaxClients; i++)
	{
		if (!IsClientInGame(i)) continue;
		
		if ((GetClientTeam(i) == 3 && GetConVarBool(g_CvarInfectedVision)) /* Client in Infected Team and Convar enabled */
			|| (IsClientObserver(i) && GetConVarBool(g_CvarSpectatorVision))) /* Client in Spectator Team and Convar enabled */
		{
			bVision[i] = true;
		}
		else
...
public ClearVisionEvent(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (client && IsClientInGame(client) && !IsFakeClient(client))
	{
		...
		if ((newteam == 3 && GetConVarBool(g_CvarInfectedVision)) /* Client in Infected Team and Convar enabled */
			|| (newteam == 1 && GetConVarBool(g_CvarSpectatorVision))) /* Client in Spectator Team and Convar enabled */
		{
			bVision[client] = true;
		}
		else
...
public L4D2_OnTankPassControl(oldTank, newTank, passCount)
{
	...
	if (oldTank && !GetConVarBool(g_CvarInfectedVision))
		bVision[oldTank] = false;
		
	if (bTankAlive) RecreateHittableClones(false);
}
...
public RecreateHittableClones(bool:visible)
{
	...
	if (ArraySize > 0)
	{
		for (new i = 0; i < ArraySize; i++)
		{
			...
			if (IsTankHittable(storedEntity))
			{
				...
				{
					PushArrayCell(g_ArrayHittableClones, clone);
					MakeEntityVisible(clone, visible);
					SDKHook(clone, SDKHook_SetTransmit, CloneTransmit);
					L4D2_SetEntGlow(clone, L4D2Glow_Constant, 3250, 250, OUR_COLOR, false);
				}
			}
