...
TmpHealth g_StoredTmpHealth[MAXPLAYERS+1];
...
public void OnClientDisconnect(int client) { g_StoredTmpHealth[client].Clear(); }

public void DelayRecord(int client)
{
	if (client && IsClientInGame(client) && GetClientTeam(client) == 2)
	{
		g_StoredTmpHealth[client].Record(client);
	}
}
...
public void PlayerTeam_Event(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (client && IsClientInGame(client))
	{
		...
		{
			g_StoredTmpHealth[client].Clear();
...
void HealEntityOverTime(int client, float interval, int increment, int total)
{
    ...
    // Prevent vanilla healing
    g_StoredTmpHealth[client].Refresh(client);
...
void SwitchEventHooks(bool hook)
{
	...
	if (!hook && hooked)
	{
		if (pillhot.BoolValue || adrenhot.BoolValue) return;
