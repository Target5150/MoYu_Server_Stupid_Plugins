...
public OnPluginStart()
{
	...
	hCvarSdInwaterSurvivor = CreateConVar("l4d2_slowdown_water_survivors", "-1", "Maximum slowdown in the water for the Survivors outside of Tank fights (-1: native slowdown; 0.0: No slowdown, 0.01-2.0: 1%%-200%% slowdown)", FCVAR_NONE, true, -1.0);
	hCvarSdInwaterDuringTank = CreateConVar("l4d2_slowdown_water_survivors_during_tank", "0", "Maximum slowdown in the water for the Survivors during Tank fights (0: ignore setting; 0.01-2.0: 1%%-200%% slowdown)", FCVAR_NONE, true, 0.0);
...
public Action L4D_OnGetRunTopSpeed(int client, float &retVal)
{
	...
	// TODO: Figure out whether the survivor is adrenalined
	if (bInWater && IsSurvivor(client) && !IsLimping(client))
	{
		if (GetConVarFloat(hCvarSdInwaterDuringTank) > 0.0) 
		{
